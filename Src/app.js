...
function render(){
  ...
  $("#rows").innerHTML=out.map(r=>`<tr>
    <td>${r.id}</td><td>${esc(r.word)}</td><td><span class="badge ${norm(r.type).includes("arxaizm")?"arxaizm":"istorizm"}">${esc(r.type)}</span></td>
    <td>${esc(r.macro)}</td><td>${esc(r.micro)}</td><td>${esc(r.meaning)}</td><td>${esc(r.page)}</td>
  </tr>`).join("");
  $("#cards").innerHTML=out.map(r=>`<article class="card">
    <h4>${esc(r.word)}</h4><span class="badge ${norm(r.type).includes("arxaizm")?"arxaizm":"istorizm"}">${esc(r.type)}</span>
    <p>${esc(r.meaning)}</p>
